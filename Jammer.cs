...
public class Jammer : MonoBehaviour
{
    ...
    private void jam()
    {
        ...
        List<KeyValuePair<Vector3, float>> jammingData = new List<KeyValuePair<Vector3, float>>();
        ...
        for (float y = minY; y <= maxY; y += 1)
        {
            for (float x = minX; x <= maxX; x += 1)
            {
                ...
                if (Vector3.Distance(jammerPos, jammingCoord) >= radius)
                {
                    ...
                    data.Add(new KeyValuePair<Vector3, float>(jammingCoord, jammingValue));
                }
            }
        }
